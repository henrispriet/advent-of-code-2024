...
fn solve_part1(InputData { left, right }: &InputData) -> u32 {
  ...
  left.sort_unstable();
  right.sort_unstable();
  std::iter::zip(left, right).map(|(l, r)| l.abs_diff(r)).sum()
}

#[aoc(day1, part2)]
fn solve_part2(InputData { left, right }: &InputData) -> u32 {
  let right_count = right.iter().fold(std::collections::HashMap::new(), |mut acc, x| {
    match acc.get_mut(x) {
      Some(count) => *count += 1,
      None => { acc.insert(x, 1); }
    }
  ...
  left.iter().filter_map(|l| right_count.get(l).map(|r| l * r)).sum()
}
